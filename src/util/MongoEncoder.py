...
def dumps(obj,excluded=None,  *args, **kwargs):
...
#         print type(obj)
        if isinstance(obj,Document) or isinstance(obj,EmbeddedDocument):
            return _json_convert(obj.to_mongo(),)
        l = []
        for v in obj:
            
            if isinstance(v,Document) or isinstance(v,EmbeddedDocument):
                l.append(_json_convert(v.to_mongo(),))
            else:
                l.append(_json_convert(v,))
        return l
    try:
        return default(obj)
    except TypeError:
