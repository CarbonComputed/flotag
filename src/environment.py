...
if os.path.exists(path('handlers')):
    for directory in os.listdir(path('handlers')):
        full_path = path('handlers/%s' % directory)
        if os.path.isdir(full_path):
            site.addsitedir(full_path)
            
if os.path.exists(path('models')):
    for directory in os.listdir(path('models')):
        full_path = path('models/%s' % directory)
        if os.path.isdir(full_path):
            site.addsitedir(full_path)
