...
(function() {
  ...
  Swag.addHelper = function(name, helper) {
    return Ember.Handlebars.helper(name, helper);
  };
  ...
  Swag.addHelper('linkify', function(str) {
    var exp = /(\b(https?|ftp|file):\/\/[-A-Z0-9+&@#\/%?=~_|!:,.;]*[-A-Z0-9+&@#\/%=~_|])/ig;
    return Ember.String.htmlSafe(str.replace(exp,"<a href='$1'>$1</a>")); 
  });
...
Ember.Handlebars.registerBoundHelper('is', function(value, test, options) {
    if (value === test) {
      return options.fn(this);
    } else {
      return options.inverse(this);
...
Ember.Handlebars.registerBoundHelper('isnt', function(value, test, options) {
    if (value !== test) {
      return options.fn(this);
    } else {
      return options.inverse(this);
